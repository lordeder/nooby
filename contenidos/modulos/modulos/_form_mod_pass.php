	 	...
	 	if( isset($_POST['texto1']) and isset($_POST['texto2']) ){
			...
			mensaje_error('La contraseña es incorrecta','');
				...
				if(mod_1_dato($tabla_usuarios,'password',$pass_in,'userid',$userid,$link)){
					mensaje_ok(' Se modificó su contraseña ','x');
					$process=true;
				}else{
...
<input name="texto_3m_p" id="texto_3m_p" type="password" placeholder="Contraseña actual" class="form-control"  /><br />
 ...
 <center> <button type="submit" class="btn btn-primary" onclick="ing('div_mod_pass','<?php echo $id; ?>', 'mod_pass', 'm_p', 'add')">Modificar</button></center>
